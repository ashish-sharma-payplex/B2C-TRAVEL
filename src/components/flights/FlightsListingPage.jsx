...
const FlightDetailSidebar = ({ open, onClose, flight, searchMeta }) => {
  ...
  const handleContinue = () => {
    ...
    navigate("/book-flight", {
      state: { flight, searchMeta },
    });
  };

  // ─── Single Segment Block (used for both no-stop and each leg of stops) ───
  ...
  // ─── Layover / Change of planes badge ────────────────────────────────────
  // groundTime may live on the NEXT segment; fallback to calculating from timestamps
  const LayoverBadge = ({ currentSeg, nextSeg }) => {
    let groundTime = nextSeg?.GroundTime || currentSeg?.GroundTime || 0;

    // Fallback: calculate from arr → dep timestamps if groundTime is 0
    if (!groundTime && currentSeg?.Destination?.ArrTime && nextSeg?.Origin?.DepTime) {
      ...
      groundTime = Math.round((depMs - arrMs) / 60000); // convert ms → minutes
    }
    ...
    return (
      ...
      >
        ...
        />
        <Box>
          <Typography sx={{ fontSize: 13, fontWeight: 700, color: "#166534", lineHeight: 1.3 }}>
            Change of planes
          </Typography>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/*
          ─── Segments rendering ───────────────────────────────────────────────
          No-stop  (segs.length === 1): renders exactly as before — single SegmentBlock, no layover badge.
          1 stop   (segs.length === 2): SegmentBlock → LayoverBadge → SegmentBlock
          2+ stops (segs.length  > 2): SegmentBlock → LayoverBadge → SegmentBlock → LayoverBadge → ...
        */}
        {segs.map((seg, idx) => (
          <React.Fragment key={idx}>
            <SegmentBlock seg={seg} />
            {/* Show layover badge AFTER each segment except the last */}
            {idx < segs.length - 1 && (
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
          <Box sx={{ display: "flex", justifyContent: "space-between" }}>
            ...
            <Typography sx={{ fontSize: 13, color: "#222", fontWeight: 500 }}>
              ₹ {(flight.Fare?.BaseFare * totalPassengers).toLocaleString("en-IN")}
            </Typography>
          </Box>
          <Box sx={{ display: "flex", justifyContent: "space-between" }}>
        ...
        <Box sx={{ mt: 2, display: "flex", gap: 1, flexWrap: "wrap" }}>
          ...
          {segs[0]?.SupplierFareClass && (
            ...
            >
              <Typography sx={{ fontSize: 11.5, fontWeight: 600, color: "#1D4ED8" }}>
                {segs[0].SupplierFareClass}
              </Typography>
            </Box>
          )}
...
const getFlightMeta = (flight) => {
  ...
  const last = segs[segs.length - 1];
  const depTime = first.Origin?.DepTime?.split("T")[1]?.substring(0, 5) || "--";
  const arrTime = last.Destination?.ArrTime?.split("T")[1]?.substring(0, 5) || "--";
  const totalDuration = last.AccumulatedDuration || first.Duration || 0;
  const durationHours = Math.floor(totalDuration / 60);
  const durationMinutes = totalDuration % 60;
  const stopsCount = segs.length - 1;
...
const FlightsListingPage = () => {
  ...
  const uniqueAirlines = useMemo(() => getUniqueAirlines(rawFlights), [rawFlights]);
  ...
  const filteredFlights = useMemo(() => {
    return rawFlights.filter((flight) => {
      ...
      if (selectedAirlines.length > 0 && !selectedAirlines.includes(meta.airlineName))
        return false;
      return true;
    });
  }, [rawFlights, effectivePriceRange, selectedStops, departureTime, arrivalTime, selectedAirlines]);
  ...
  const FilterPanel = () => (
    ...
    >
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
        <Typography sx={{ fontSize: 17, fontWeight: 600, color: "#383E48" }}>Filter by:</Typography>
        <Typography
          onClick={clearAll}
          sx={{ color: "#1A914B", textDecoration: "underline", cursor: "pointer", fontSize: 13, fontWeight: 500 }}
        >
          Clear
        </Typography>
      </Box>

      <Typography sx={{ fontSize: 15, fontWeight: 600, mb: 1.5, color: "#222" }}>Price Range</Typography>
      <Slider
        value={effectivePriceRange}
        onChange={(_, v) => setPriceRange(v)}
        min={minP}
        max={maxP}
        sx={{ color: "#1A914B", "& .MuiSlider-thumb": { width: 18, height: 18 } }}
      />
      <Box sx={{ display: "flex", justifyContent: "space-between", mb: 2 }}>
        <Typography sx={{ fontSize: 13, color: "#555" }}>₹ {effectivePriceRange[0].toLocaleString("en-IN")}</Typography>
        <Typography sx={{ fontSize: 13, color: "#555" }}>₹ {effectivePriceRange[1].toLocaleString("en-IN")}</Typography>
      </Box>

      <Divider sx={{ borderStyle: "dashed", mb: 2 }} />

      <Typography sx={{ fontSize: 15, fontWeight: 600, mb: 1, color: "#222" }}>Stops</Typography>
      {["Nonstop", "1 Stop", "2+ Stop"].map((stop) => (
        <FormControlLabel
          key={stop}
          control={
          ...
          label={<Typography sx={{ fontSize: 13.5, color: "#444" }}>{stop}</Typography>}
          sx={{ ml: 0, display: "flex" }}
        />
      ))}
      ...
      <Typography sx={{ fontSize: 15, fontWeight: 600, mb: 1.5, color: "#222" }}>
        Departure from {searchMeta.fromCity?.code || "Origin"}
      </Typography>
      <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1.2, mb: 2 }}>
        {timeSlots.map(({ label, icon }) => (
          <Paper
            key={label}
            onClick={() => setDepartureTime(departureTime === label ? "" : label)}
            elevation={0}
            sx={{
              p: 1.2,
              textAlign: "center",
              borderRadius: "10px",
              cursor: "pointer",
              border: departureTime === label ? "1.5px solid #1A914B" : "1px solid #E3E8EE",
              backgroundColor: departureTime === label ? "#EAF7EF" : "#fff",
              display: "flex",
              flexDirection: "column",
              alignItems: "center",
          ...
          >
            <Box sx={{ color: departureTime === label ? "#1A914B" : "#888" }}>{icon}</Box>
            <Typography sx={{ fontSize: 11, fontWeight: 500, color: departureTime === label ? "#1A914B" : "#555", lineHeight: 1.3 }}>
              {label}
            </Typography>
          </Paper>
        ))}
        ...
        {timeSlots.map(({ label, icon }) => (
          <Paper
            ...
            sx={{
              ...
              borderRadius: "10px",
              cursor: "pointer",
              border: arrivalTime === label ? "1.5px solid #1A914B" : "1px solid #E3E8EE",
              backgroundColor: arrivalTime === label ? "#EAF7EF" : "#fff",
              display: "flex",
              flexDirection: "column",
              alignItems: "center",
          ...
          >
            <Box sx={{ color: arrivalTime === label ? "#1A914B" : "#888" }}>{icon}</Box>
            <Typography sx={{ fontSize: 11, fontWeight: 500, color: arrivalTime === label ? "#1A914B" : "#555", lineHeight: 1.3 }}>
              {label}
            </Typography>
          </Paper>
        ))}
      ...
      <Typography sx={{ fontSize: 15, fontWeight: 600, mb: 1, color: "#222" }}>Airlines</Typography>
      {uniqueAirlines.map((airline) => (
        <FormControlLabel
          key={airline}
          control={
            <Checkbox
              ...
              sx={checkboxStyle}
              checked={selectedAirlines.includes(airline)}
              onChange={() => toggleItem(setSelectedAirlines, selectedAirlines, airline)}
            />
          }
          label={
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              <AirlineLogo name={airline} code="" size={24} />
              <Typography sx={{ fontSize: 13.5, color: "#444" }}>{airline}</Typography>
            </Box>
          }
          sx={{ ml: 0, display: "flex" }}
        />
    ...
    return (
      <Paper
        ...
        sx={{
          ...
          border: "1px solid #E3E8EE",
          bgcolor: "#fff",
          "&:hover": { boxShadow: "0 4px 16px rgba(0,0,0,0.08)", borderColor: "#ccc" },
          transition: "box-shadow .2s, border-color .2s",
        }}
      >
        <Box
          sx={{
            display: "grid",
            gridTemplateColumns: { xs: "1fr", sm: "1.8fr 1.2fr 1.4fr 1.2fr 1.4fr" },
            alignItems: "center",
            gap: { xs: 1.5, sm: 0 },
          }}
        >
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
            <AirlineLogo name={airlineName} code={airlineCode} size={44} />
            <Box>
              <Typography sx={{ fontSize: 14.5, fontWeight: 600, color: "#222" }}>{airlineName}</Typography>
              <Typography sx={{ fontSize: 12.5, color: "#888" }}>
                {airlineCode}-{flightNumber}
                {stopsCount > 0 && ` · ${stopsCount} stop${stopsCount > 1 ? "s" : ""}`}
              </Typography>
              <Box sx={{ display: "flex", gap: 0.5, mt: 0.3, flexWrap: "wrap" }}>
                <Typography sx={{ fontSize: 11, color: flight.IsRefundable ? "#1A914B" : "#E57373", fontWeight: 500 }}>
                  {flight.IsRefundable ? "Refundable" : "Non-refundable"}
                </Typography>
                {fareClass && <Typography sx={{ fontSize: 11, color: "#888" }}>· {fareClass}</Typography>}
              </Box>
            </Box>
          </Box>
          ...
          {/* Departure */}
          <Box>
            <Typography sx={{ fontSize: { xs: 13, sm: 17 }, fontWeight: 700, color: "#222" }}>{depTime}</Typography>
            <Typography sx={{ fontSize: 12.5, color: "#888" }}>{originCity}</Typography>
          </Box>
          ...
          <Box sx={{ textAlign: { xs: "left", sm: "center" } }}>
            ...
            >
              ...
              {segs.slice(0, -1).map((_, i) => (
                <React.Fragment key={i}>
                  <Box sx={{ flex: 1, height: 1, bgcolor: "#E5E7EB", maxWidth: 40 }} />
                  <Box sx={{ width: 6, height: 6, borderRadius: "50%", bgcolor: "#9CA3AF" }} />
                </React.Fragment>
              ))}
              <Box sx={{ flex: 1, height: 1, bgcolor: "#E5E7EB", maxWidth: 40 }} />
              <Box sx={{ width: 6, height: 6, borderRadius: "50%", bgcolor: "#D1D5DB" }} />
            </Box>
            <Typography sx={{ fontSize: 11.5, color: "#888" }}>{stopLabel}</Typography>
            {stopsCount > 0 && (
              <Typography sx={{ fontSize: 10.5, color: "#aaa" }}>
                via {segs.slice(0, -1).map((s) => s.Destination?.Airport?.CityCode).join(", ")}
              </Typography>
            )}
          </Box>
          ...
          {/* Arrival */}
          <Box>
            <Typography sx={{ fontSize: { xs: 13, sm: 17 }, fontWeight: 700, color: "#222" }}>{arrTime}</Typography>
            <Typography sx={{ fontSize: 12.5, color: "#888" }}>{destCity}</Typography>
          </Box>
          ...
          >
            <Box sx={{ textAlign: { xs: "left", sm: "right" } }}>
              <Typography sx={{ fontSize: { xs: 17, sm: 20 }, fontWeight: 700, color: "#222" }}>
                ₹ {price?.toLocaleString("en-IN")}
              </Typography>
              <Typography sx={{ fontSize: 11, color: "#6B7280" }}>
                {seatsLeft <= 5 ? (
                  <span style={{ color: "#E57373", fontWeight: 600 }}>{seatsLeft} seats left!</span>
                ) : (
                  `${seatsLeft} seats`
                )}
              </Typography>
  ...
  return (
    <Box sx={{ bgcolor: "#F5F7FA", minHeight: "100vh" }}>
      ...
      {/* Header info bar */}
      <Box sx={{ bgcolor: "#fff", borderBottom: "1px solid #E3E8EE", px: { xs: 2, md: 4 }, py: 1.5 }}>
        <Typography sx={{ fontSize: 14, color: "#555" }}>
          <strong style={{ color: "#111" }}>{searchMeta.fromCity?.name || searchMeta.fromCity?.code || "Origin"}</strong>
          {" → "}
          <strong style={{ color: "#111" }}>{searchMeta.toCity?.name || searchMeta.toCity?.code || "Destination"}</strong>
          {searchMeta.departureDate && (
            <span style={{ marginLeft: 8, color: "#888" }}>{new Date(searchMeta.departureDate).toDateString()}</span>
          )}
          {searchMeta.passengers && (
            <span style={{ marginLeft: 8, color: "#888" }}>
              · {searchMeta.passengers.adults} Adult
              {searchMeta.passengers.children ? `, ${searchMeta.passengers.children} Child` : ""}
            </span>
          )}
          <span style={{ marginLeft: 8, color: "#1A914B", fontWeight: 600 }}>
            {rawFlights.length} flights found
      ...
      <Box sx={{ display: { xs: "flex", md: "none" }, px: 2, pt: 2, gap: 1 }}>
        ...
        <Typography sx={{ fontSize: 13.5, color: "#555", alignSelf: "center", ml: 1 }}>
          {filteredFlights.length} results
        </Typography>
      </Box>
      ...
      >
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          {filteredFlights.length > 0 && (
            ...
            >
              {["Airline", "Departure", "Duration", "Arrival", "Price"].map((h) => (
                <Typography
                  key={h}
                  sx={{ fontSize: 12.5, color: "#9CA3AF", fontWeight: 500, textAlign: h === "Price" ? "right" : "left" }}
                >
                  {h}
                </Typography>
              ))}
          ...
          {filteredFlights.length === 0 ? (
            <Paper
              elevation={0}
              sx={{ p: 5, textAlign: "center", borderRadius: "14px", border: "1px solid #E3E8EE", bgcolor: "#fff" }}
            >
              <Typography sx={{ fontSize: 17, color: "#555", fontWeight: 500 }}>
                {rawFlights.length === 0 ? "No flights data received" : "No flights match your filters"}
              </Typography>
              <Typography sx={{ fontSize: 13.5, color: "#9CA3AF", mt: 1 }}>
                {rawFlights.length === 0 ? "Please search again" : "Try adjusting your filters"}
              </Typography>
            </Paper>
          ) : (
            filteredFlights.map((flight, i) => (
