...
import {
  Box, Paper, List, ListItem, ListItemButton, ListItemIcon, ListItemText,
  Button, Typography, Card, CardContent, Divider, Dialog, DialogContent,
  IconButton, Grid, Drawer, useMediaQuery, useTheme,
} from "@mui/material";
...
const TicketSection = ({ title, children }) => (
  <Paper
    elevation={0}
    sx={{
      p: 2.5, border: "1px solid #e5e7eb", borderRadius: 3, height: "170px",
      display: "flex", flexDirection: "column", justifyContent: "flex-start",
      boxSizing: "border-box", transition: "0.2s ease",
      "&:hover": { boxShadow: "0 4px 10px rgba(0,0,0,0.08)" },
    }}
  >
    <Typography sx={{ fontWeight: 700, fontSize: 16, color: "#166534", mb: 2 }}>{title}</Typography>
    {children}
  </Paper>
);
    ...
    <CardContent sx={{ p: 2, "&:last-child": { pb: 2 } }}>
      ...
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 1.5 }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          <Box sx={{ width: 34, height: 34, borderRadius: "50%", bgcolor: "#f3f4f6" }} />
...
const STATIC_DATA = {
  Trains: [
    { id: 3, bookingId: "TR-2024-001", from: "Delhi (DLI)", to: "Agra (AGR)", date: "2024-07-20", time: "06:00", train: "Rajdhani Express", status: "Upcoming", price: "₹1,200" },
    { id: 4, bookingId: "TR-2024-002", from: "Mumbai (CSMT)", to: "Pune (PHC)", date: "2024-06-10", time: "15:30", train: "Intercity Express", status: "Cancelled", price: "₹800" },
  ],
  Hotels: [
    { id: 5, bookingId: "HT-2024-001", name: "Taj Mahal Hotel", location: "Delhi", checkIn: "2024-07-15", checkOut: "2024-07-18", nights: 3, status: "Upcoming", price: "₹15,000/night" },
    { id: 6, bookingId: "HT-2024-002", name: "Oberoi Mumbai", location: "Mumbai", checkIn: "2024-06-01", checkOut: "2024-06-03", nights: 2, status: "Past", price: "₹12,000/night" },
  ],
  Cabs: [
    { id: 7, bookingId: "CB-2024-001", pickupLocation: "Mumbai Airport", dropLocation: "Bandra", date: "2024-07-16", time: "18:00", carType: "Sedan", status: "Upcoming", price: "₹800" },
    { id: 8, bookingId: "CB-2024-002", pickupLocation: "Colaba", dropLocation: "Fort", date: "2024-06-15", time: "10:30", carType: "Hatchback", status: "Failed", price: "₹400" },
  ],
};
...
const MyTrips = () => {
  ...
  const visibleData = selectedCategory === "Flights"
    ...
    <>
      <Box sx={{ p: 2.5, pb: 1.5 }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          ...
          <Typography sx={{ fontSize: 18, fontWeight: 700, color: "#1a1a1a" }}>My Trips</Typography>
        </Box>
      </Box>
      <Divider />
      <List sx={{ p: 0 }}>
        {CATEGORIES.map((category) => (
          <ListItem
            ...
            sx={{ bgcolor: selectedCategory === category.label ? "#f0fdf4" : "transparent" }}
          >
            <ListItemButton
              sx={{
                py: 1.5, px: 2,
                borderLeft: selectedCategory === category.label ? `3px solid ${GREEN}` : "3px solid transparent",
                transition: "all 0.2s ease",
                "&:hover": { bgcolor: "#fafafa" },
              }}
            >
              <ListItemIcon sx={{ minWidth: 40, color: selectedCategory === category.label ? GREEN : "#999" }}>
                {category.icon}
              </ListItemIcon>
              <ListItemText
      ...
      >
        ...
        <Box
          sx={{
            ...
            opacity: 0.18,
            maskImage: "linear-gradient(to right, white 0%, black 25%, black 75%, white 100%)",
            WebkitMaskImage: "linear-gradient(to right, white 0%, black 25%, black 75%, white 100%)",
          }}
        />
        ...
        <CardContent sx={{ position: "relative", p: 2, "&:last-child": { pb: 2 } }}>
          ...
          <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 1 }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              <Box sx={{ width: 34, height: 34, borderRadius: "50%", bgcolor: "#dcfce7", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.5, px: 1.5, py: 0.4, borderRadius: "20px", fontSize: 11, fontWeight: 700, bgcolor: sc.bg, color: sc.color }}>
              {booking.status}
              {(booking.status === "BOOKED" || booking.status === "TICKETED") && <span style={{ fontWeight: 800 }}>✓</span>}
            </Box>
          </Box>

          {/* Row 2: Origin → Destination */}
          <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 1 }}>
            <Typography sx={{ fontSize: 13, fontWeight: 700, color: "#1a1a1a" }}>{booking.origin || "—"}</Typography>
            <Typography sx={{ fontSize: 12, color: "#aaa" }}>✈</Typography>
            <Typography sx={{ fontSize: 13, fontWeight: 700, color: "#1a1a1a" }}>{booking.destination || "—"}</Typography>
          </Box>
          ...
          <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 1.5 }}>
            ...
            <Typography sx={{ fontSize: 11, color: "#aaa" }}>#{booking.booking_id}</Typography>
          </Box>

          {/* Row 4: Fare */}
          {booking.fare?.published_fare && (
            <Typography sx={{ fontSize: 17, fontWeight: 800, color: GREEN, mb: 1.5, lineHeight: 1 }}>
              ₹{booking.fare.published_fare.toLocaleString("en-IN")}
              <Typography component="span" sx={{ fontSize: 11, color: "#888", fontWeight: 400, ml: 0.5 }}>paid</Typography>
            </Typography>
          )}
          ...
          <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <Button
              ...
              sx={{
                ...
                borderRadius: "8px", px: 2, py: 0.6, bgcolor: GREEN,
                boxShadow: "none", "&:hover": { bgcolor: "#15803d", boxShadow: "none" },
              }}
            >
              {detailsLoading ? "Loading..." : "View Details"}
            </Button>
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.5, cursor: "pointer", color: "#555", "&:hover": { color: GREEN } }}>
              <Typography sx={{ fontSize: 13 }}>⬇</Typography>
              <Typography sx={{ fontSize: 12, fontWeight: 500 }}>E-Ticket</Typography>
            </Box>
          </Box>
        </CardContent>
      </Card>
  ...
  const renderStaticCard = (booking) => (
    <Card
      ...
      elevation={0}
      sx={{
        mb: 2, border: "1px solid #eee", borderLeft: `5px solid ${GREEN}`,
    ...
    >
      <CardContent sx={{ p: { xs: 2, sm: 3 }, "&:last-child": { pb: { xs: 2, sm: 3 } } }}>
        <Box sx={{ display: "flex", justifyContent: "flex-end", mb: 1.5 }}>
          <Box sx={{
            px: 2, py: 0.7, borderRadius: "20px", fontSize: 13, fontWeight: 700,
            bgcolor: booking.status === "Upcoming" ? "#fef3c7" : booking.status === "Past" ? "#dbeafe" : booking.status === "Cancelled" ? "#fee2e2" : "#f3f4f6",
            color: booking.status === "Upcoming" ? "#b45309" : booking.status === "Past" ? "#0369a1" : booking.status === "Cancelled" ? "#dc2626" : "#374151",
          }}>
            {booking.status}
          </Box>
        </Box>
          ...
          <>
            <Box sx={{ display: "flex", justifyContent: "space-between", mb: 1 }}>
              <Box>
                ...
                <Typography sx={{ fontWeight: 600, color: "#1a1a1a" }}>{booking.from} → {booking.to}</Typography>
              </Box>
            ...
            <Box sx={{ display: "flex", gap: 3 }}>
              <Box>
                ...
                <Typography sx={{ fontWeight: 600, color: "#1a1a1a" }}>{booking.date} • {booking.time}</Typography>
              </Box>
          ...
          <>
            ...
            <Box sx={{ display: "flex", justifyContent: "space-between" }}>
              <Box>
                ...
                <Typography sx={{ fontWeight: 600, color: "#1a1a1a", fontSize: 13 }}>{booking.checkIn} / {booking.checkOut}</Typography>
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          {booking.status === "Upcoming" && (
            <Button size="small" variant="outlined" sx={{ borderColor: "#ccc", color: "#666", fontWeight: 600, fontSize: 12, textTransform: "none", "&:hover": { borderColor: "#999" } }}>
              Modify
            </Button>
          )}
        </Box>
    ...
    <>
      <Box sx={{ minHeight: "100vh", bgcolor: "#f5f5f5", py: 3 }}>
        <Box sx={{ maxWidth: 1400, mx: "auto", px: { xs: 1.5, sm: 2, md: 4 } }}>
          ...
          {/* Top bar */}
          <Box sx={{ display: "flex", alignItems: "center", gap: { xs: 1, sm: 2 }, mb: 3, justifyContent: { xs: "space-between", md: "flex-end" }, flexWrap: "wrap" }}>
            {isMobile && (
              <Button variant="outlined" size="small" startIcon={<MenuIcon />} onClick={() => setDrawerOpen(true)}
                sx={{ borderColor: GREEN, color: GREEN, textTransform: "none", fontWeight: 600, fontSize: 13, "&:hover": { bgcolor: "#f0fdf4" } }}>
                Categories
              </Button>
            )}
              ...
              <Box sx={{ display: "flex", gap: { xs: 0.8, sm: 2 }, flexWrap: "wrap" }}>
                {STATUS_FILTERS.map((status) => (
                  <Button
                    ...
                    onClick={() => { setSelectedStatus(status); setVisibleCount(6); }}
                    sx={{
                      borderRadius: "20px", fontWeight: 600, fontSize: 13, textTransform: "none", px: 2.5, py: 1, minWidth: 0,
            ...
            {isMobile && (
              <Button variant="outlined" startIcon={<SortIcon />} onClick={() => setSortDrawerOpen(true)}
                sx={{ borderColor: GREEN, color: GREEN, textTransform: "none", fontWeight: 600, fontSize: 13, borderRadius: "10px", px: 2, "&:hover": { borderColor: GREEN, bgcolor: "#f0fdf4" } }}>
                Sort
              </Button>
            )}
          </Box>
          ...
          <Box sx={{ display: "flex", gap: 3, minHeight: "calc(100vh - 150px)", alignItems: "flex-start" }}>
            ...
            {/* Sidebar desktop */}
            {!isMobile && (
              <Paper elevation={0} sx={{ width: 300, bgcolor: "#ffffff", borderRadius: "16px", border: "1px solid #e8e8e8", flexShrink: 0, height: "fit-content", position: "sticky", top: 24 }}>
                <SidebarContent />
              </Paper>
            )}

            {/* Mobile drawer - categories */}
            <Drawer anchor="left" open={drawerOpen} onClose={() => setDrawerOpen(false)} PaperProps={{ sx: { width: 260, borderRadius: "0 16px 16px 0" } }}>
            ...
            {/* Sort drawer */}
            <Drawer anchor="right" open={sortDrawerOpen} onClose={() => setSortDrawerOpen(false)} PaperProps={{ sx: { width: 220 } }}>
              <Box sx={{ p: 2, display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #eee" }}>
                <Typography sx={{ fontWeight: 700, fontSize: 18 }}>Sort</Typography>
                <IconButton onClick={() => setSortDrawerOpen(false)}><CloseIcon /></IconButton>
              </Box>
              <List sx={{ p: 0 }}>
                {STATUS_FILTERS.map((status) => (
                  <ListItem key={status} disablePadding>
                    <ListItemButton onClick={() => handleSortSelect(status)} sx={{ py: 1.8, bgcolor: selectedStatus === status ? "#f0fdf4" : "transparent" }}>
                      <ListItemText primary={status} primaryTypographyProps={{ fontWeight: selectedStatus === status ? 700 : 500, color: selectedStatus === status ? GREEN : "#333" }} />
                    </ListItemButton>
                  </ListItem>
                ))}
              </List>
            ...
            {/* Right content */}
            <Paper elevation={0} sx={{ flex: 1, bgcolor: "#ffffff", borderRadius: "16px", border: "1px solid #e8e8e8", p: { xs: 2, sm: 3, md: 4 }, minHeight: "600px", minWidth: 0 }}>
              ...
              {isMobile && (
                <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 2, pb: 1.5, borderBottom: "1px solid #f3f4f6" }}>
                  ...
                  <Typography sx={{ fontWeight: 700, fontSize: 15, color: "#1a1a1a" }}>{selectedCategory}</Typography>
                </Box>
              )}
              ...
              {selectedCategory === "Bus" ? (
                    ...
                    {!loadingMore && visibleCount < filteredData.length && (
                      <Box sx={{ display: "flex", justifyContent: "center", mt: 3 }}>
                        <Button
                          ...
                          sx={{
                            borderColor: GREEN, color: GREEN, fontWeight: 600, fontSize: 14,
                            textTransform: "none", borderRadius: "10px", px: 4, py: 1,
                ...
                <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "500px", textAlign: "center", px: 2 }}>
                  ...
                  <Button variant="contained" sx={{ mt: 3, bgcolor: GREEN, textTransform: "none", fontWeight: 600, "&:hover": { bgcolor: "#15803d" } }}>
                    Book your next trip
                  </Button>
                </Box>
              )}
      ...
      <Dialog open={openDetails} onClose={handleCloseDetails} fullWidth maxWidth="lg"
        PaperProps={{ sx: { borderRadius: 6, overflow: "hidden", width: "100%", maxWidth: "950px", maxHeight: "85vh", boxShadow: "0 20px 50px rgba(0,0,0,0.15)", mx: { xs: 1, sm: 2 } } }}>
            ...
            return (
              <Box>
                <Box sx={{ bgcolor: "#16a34a", color: "#fff", p: 3 }}>
                  <Typography sx={{ fontSize: 24, fontWeight: 700 }}>✈️ Flight Ticket</Typography>
                  ...
                  <Grid container spacing={3} alignItems="stretch">
                    <Grid item xs={12} md={6}>
                      <TicketSection title="Flight Details">
                        ...
                        <Typography>Route : {selectedBooking.origin} → {selectedBooking.destination}</Typography>
                      </TicketSection>
                    </Grid>
                    <Grid item xs={12} md={6}>
                      <TicketSection title="Passenger Details">
                        <Typography>{passenger?.Title} {passenger?.FirstName} {passenger?.LastName}</Typography>
                    ...
                    <Grid item xs={12} md={6}>
                      <TicketSection title="Departure">
                        ...
                        <Typography>{new Date(segment?.Origin?.DepTime).toLocaleString()}</Typography>
                      </TicketSection>
                    </Grid>
                    <Grid item xs={12} md={6}>
                      <TicketSection title="Arrival">
                        <Typography>{segment?.Destination?.Airport?.CityName}</Typography>
                        <Typography>{new Date(segment?.Destination?.ArrTime).toLocaleString()}</Typography>
                      </TicketSection>
                    </Grid>
                    <Grid item xs={12} md={6}>
